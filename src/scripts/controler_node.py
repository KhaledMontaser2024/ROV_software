...
class controller():
    def __init__(self) -> None:
        ...
        command_publisher=rospy.Publisher("/Controller_to_Caymen",ButtonsStateMessage,queue_size=10)
        ...
        #### to use later in L2 R2 axis compination ######
        self.L2=float()
        self.R2=float()
        ##################################################
    ...
    def Buttons(self,event):
        ...
        self.ButtonsState.L1 ^= event.button == self.keys['L1']
        self.ButtonsState.R1 ^= event.button == self.keys['R1']
    ...
    def Analogs(self,event):
        ...
        self.ButtonsState.R2_L2_Axis = self.R2 + self.L2
        ######################################################################################



        ############### right and left analogs manipulation ##################################
        if event.axis == self.keys['RightAnalog_Up_Down_Axis']:
            self.ButtonsState.RightAnalog_Up_Down_Axis = round(-event.value,4)
        if event.axis == self.keys['RightAnalog_Left_Right_Axis']:
            self.ButtonsState.RightAnalog_Left_Right_Axis = round(event.value,4)
        if event.axis == self.keys['LeftAnalog_Up_Down_Axis']:
            self.ButtonsState.LeftAnalog_Up_Down_Axis = round(-event.value,4)
        if event.axis == self.keys['LeftAnalog_Left_Right_Axis']:
            self.ButtonsState.LeftAnalog_Left_Right_Axis = round(event.value,4)
        #######################################################################################
